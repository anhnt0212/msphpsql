...
#define ALLOC_ZVAL(z)	\
	...
	// This can only be used with the existing smart pointers
	// Or zval_ptr_dtor should be called
#define ALLOC_INIT_ZVAL(zp)						\
	ALLOC_ZVAL(zp);		

	// Added for testing and possibly future benchmark purposes
#define ALLOC_HASHTABLE_PERSISTENT(ht)	do {									\
	ZVAL_NEW_PERSISTENT_ARR(ht);														\
	_zend_hash_init(Z_ARRVAL_P(ht), 0, ZVAL_PTR_DTOR, 1 ZEND_FILE_LINE_RELAY_CC);	\
	} while (0)

	// Find a zend resource in the global resource list of Zend engine
	// and return it by zval
	inline zval* zend_list_find(int id)
	{
		zval* le = NULL;
		le = zend_hash_index_find(&EG(regular_list), id);
		return le;
	}
